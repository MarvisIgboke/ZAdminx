...
import { FlaggedMetersPage } from "./pages/flagged";
  ...
  return (
    <div className="mx-auto max-w-xl">
      <Card className="anim-rise p-6 text-center">
        ...
        <p className="mt-1 text-[12.5px] text-mute">{note ?? (<>Your role does not include <code className="rounded bg-paper px-1 font-mono text-[11px] font-bold">{perm}</code>.</>)} Authorization is enforced on every transition, not just in the UI.</p>
...
function Router() {
  ...
  switch (head) {
    ...
    case OPS.flag.path: page = can("flag.view") ? (second ? <OperationDetailPage type="flag" id={second} /> : <FlaggedMetersPage />) : <AccessDenied perm="flag.view" />; break;
    ...
    case "zvend": page = user.role === "IT_MANAGER" || user.role === "SUPER_ADMIN" ? <ZVendIntegrationPage /> : <AccessDenied perm="zvend.console" note="The ZVend wire contract is restricted to the IT Manager and Super Admin." />; break;
    case "api-docs": page = user.role === "SUPER_ADMIN" ? <ApiReferencePage /> : <AccessDenied perm="api_reference.view" />; break;
    case "admin":
      if (second === "database") page = can("admin.database") ? <DatabaseSetupPage /> : <AccessDenied perm="admin.database" />;
      else page = can("admin.users") || can("admin.api") || can("admin.audit") || can("admin.settings") ? <AdminPage tab={second} /> : <AccessDenied perm="admin.users" />;
      break;
    default: page = (
      <div className="mx-auto max-w-xl">
        <Card className="anim-rise p-6 text-center">
